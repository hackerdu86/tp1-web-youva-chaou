...
function Card(teacherInformations) {
    return (
        <div className="card">  
            ...
            <h3 className="card-text">{teacherInformations.taughtClassroomsList.map((classroom) => {
                return classroom += (teacherInformations.taughtClassroomsList.indexOf(classroom) < teacherInformations.taughtClassroomsList.length - 1) ? ", ":"";
            })}</h3>
        </div>
    );
}
